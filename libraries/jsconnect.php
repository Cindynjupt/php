 ...
 *
 * @param array $User
 *        	An array containing information about the currently signed on user. If no user is signed in then this should be an empty array.
 * @param array $Request
 *        	An array of the $_GET request.
 ...
 *        	Whether or not to check for security. This is one of these values.
...
class Jsconnect
{
	function WriteJsConnect($User, $Request, $ClientID, $Secret, $Secure = TRUE)
	{
		...
		if ($Secure)
		{
			...
			else
			{
				...
				if ($Signature != $Request ['signature']) $Error = array (
                                        'error' => 'access_denied',
	 ...
	 *        	The string to hash.
	 * @param string|bool $Secure
	 *        	The hash algorithm to use. TRUE means md5.
	 * @return string
	 * @since 1.1b
	 */
	function JsHash($String, $Secure = TRUE)
	{
		if ($Secure === TRUE) $Secure = 'md5';
