...
function checkall(){
  ...
  if(birthday=='' || birthday.trim().length==0){
    ...
    $('#dateInfo').html('生日不能为空');
  }

  if(university==''){

    $('#univInfo').html('学校不能为空');
  }

  if(colleage==''){

    $('#colleageInfo').html('专业不能为空');
  }

  if(admyear==0){
    $('#admyearInfo').html('入学时间不能为空');

  }
  if(graduate==''){

    $('#graduateInfo').html('学历不能为空');
  }

  if(category==''){

    $('#categoryInfo').html('学科不能为空');
  }
  if(edumail=='' || edumail.trim().length==0){
  ...
  $.ajax({
   url: "../commit_regist",
   type: "post",
   data: {'realname':realname,'username':username,'sex':sex,'birthday':birthday,'university':university,'edumail':edumail,
   'telephone':telephone,'md5pwd':md5pwd,'graduate':graduate,'colleage':colleage,'admyear':admyear,'category':category},
   cache: 'false',
   dataType: "json",
   async: 'true',
   success: function(data) {
